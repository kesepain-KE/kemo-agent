...
`session_lifecycle_sweep`，默认每小时执行一次。任务按用户扫描超过 24 小时未更新的未关闭会话，
单轮最多处理 50 个；`config/global_config.json` 的 `cron.session_idle_close_seconds` 可覆盖闲置阈值，
但不会低于 3600 秒，非法值回退为 24 小时。正在运行、排队、停止中的会话、持有未过期 Web lease
的会话以及无法确认租约状态的会话都会跳过。每个候选先登记记忆提取队列，再关闭会话；只关闭逻辑会话，
不删除历史窗口或消息，单会话失败会记录并继续处理后续候选。

CLI 正常结束、交互退出或异常收束，以及 Cron 主智能体任务结束时，也会执行同样的“记忆提取登记 →
