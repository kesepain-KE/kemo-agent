...
当前稳定版本为 `1.2.2`。`1.0.0` 完成主生态闭环，`1.0.1` 对完整框架进行了系统性稳定性
...
`1.2.2` 是稳定性和维护更新。`run/` 按职责拆成领域包，并删除旧的平铺导入路径；启动器恢复正确项目根目录，并在默认端口不可用时使用备用端口；本机 `kemo_app` 桥接跟随启动器发布的实际 Web 地址。任务计划支持修改、重试、重置、修订查看和安全回滚，Web 修改与重试按计划所属 `session_id` 隔离，任务摘要、执行记录和修订列表在返回浏览器前再次脱敏。网页结束音效按用户独立保存，只在 Windows 桌面网页端显示，并且只在显式成功完成的最终 Run 播放；发送附件后立即清除引用；运行中引导上传使用 `purpose=input`。根 `update.py` 改为薄兼容入口，实际更新能力按版本清单、远程源码、板块调度、备份、锁、回滚、构建和迁移拆入 `update/`；更新器保护 Cron/拓展运行态、拒绝组件降级并脱敏更新日志。本次同时补充项目路径、包结构、备用端口、模板和更新器安全合同测试。旧 `run.*` 平铺导入路径不再支持，外部插件需要迁移。`kemo_app` 桥接端口跟随修复记为 `1.1.5`。
