...
组件版本不要求始终与根版本相同。仅有部分板块发生变化时，只推进对应组件版本。`0.9.0` 新增运行中多模态引导：文本、图片、音频、视频和普通文件可以在 Provider/工具安全边界进入当前 Run，无法接收时会与文本一起转入下一轮，并在历史中保存结构化引导详情。因此 core 与 web 推进到 `0.9.0`；本批未修改内置子代理和工具插件实现，agents 保持 `0.8.0`，plugins 保持 `0.8.1`。
