...
组件版本不要求在后续版本中始终与根版本相同；仅有部分板块发生变化时，可以只推进对应组件版本。`1.0.0` 是主生态首次完整发布，`1.0.1` 是系统性稳定性复核版本。`1.0.2` 同时修改 core 文档与公共说明、内置 agents 记忆指引、plugins 搜索实现及 Web 公共包版本，因此根版本与四个组件统一推进。
