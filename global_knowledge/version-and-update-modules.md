...
`1.2.4` 在 1.2.3 的稳定性基础上增加外部智能体绑定和前端重试状态收口：

- 网页配置页明确解释默认、轻量和推理三个子智能体模型档位分别服务于普通代理、摘要/上下文压缩/临时记忆整理，以及任务计划/自我改进/深度分析。
- 重试气泡在下一次尝试产生思考、正文或工具进度时立即消失，成功终态仍会再次清理；失败尝试的临时状态不会残留在输入框上方。
- 授权拓展可通过 `agent_bridge.json` 声明外部代理的公开名称、输入/输出 Schema、命令和超时；`subagent_dispatch` 以 `external:<scope>:<module>:<name>` 统一发现和同步调用。
- 外部连接继续由拓展隔离进程负责，核心复用白名单、路径、取消、超时和结果大小边界；远程地址、Token、密码和 Cookie 不进入桥接清单、Prompt、回复或日志。
- 当前不开放外部代理后台 `wait=false`，待未来形成持久任务 ID、状态、取消、恢复和幂等合同后再单独扩展。
