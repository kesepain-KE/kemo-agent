...
> 当前稳定版本：`kemo-agent 1.2.8`，配套 Kemo 网关为 `kemo-adapter-api 0.8.0`。本版本聚焦多用户 Web 工作区与运行可靠性：长任务计划跨 Run 连续执行、子代理工具卡片进度、消息跟进队列、模块与 Prompt 规范、文件排序、临时重要记忆失效解释、分类运行日志与有界内存缓存、定时任务历史生命周期、新建用户标签页、启动对话空间巡检，以及暂停/停止状态竞态修复；模块密钥不得从框架根环境隐式继承，也不得写入浏览器、Prompt、历史或日志。遇到旧文档与本段冲突时，以当前代码和本段的安全规则为准。
...
- Expand/Sense 的目录模块以独立环境启动：只保留 PATH、系统目录、临时目录、当前 Python 环境和证书路径等运行必需项，不继承 Provider/Web 密钥。模块专用变量放在模块目录自己的 `.env`；框架动态选出的本机 Web 地址作为受信运行时值单独传入，不能被模块中的陈旧值覆盖。
...
- 技能标题和插件 `## Tool` 只识别 Markdown 正文中的真实标题；围栏代码和四空格缩进代码中的 `#` 仅是示例内容，不能改变发现结果。
- 插件标题、工具名、目录名三者必须一致。
...
- Shell 的 `shell_type=auto` 必须先按平台和明确语法解析为单个非登录解释器，再以 `shell=False` 启动；Windows 只检查未加引号的命令位置，不能因普通程序参数里的 PowerShell 文本切换解释器。调用级与会话级 `PATH` / `COMSPEC` 必须参与同步、后台共用的解释器解析，Windows 相对 `PATH` 以命令工作目录为基准。macOS/Linux 使用各自的 zsh/bash/sh 优先级。需要 `cmd`、`pwsh`、`powershell`、`sh`、`bash`、`zsh`、`fish` 或登录环境时可以显式指定；显式类型不可用时直接报错，不能静默改用另一种语法。
...
- Web 中所有 Markdown 入口统一使用 `web/frontend/src/markdownLinks.ts` 提供的 GFM 插件，业务页面不能直接导入 `remark-gfm`。共享插件只修正以 `http://`、`https://` 或 `www.` 开头的裸链接与中文标点或界定符相邻时的边界；显式 Markdown 链接、尖括号链接、图片和代码保持原样。聊天链接继续执行协议过滤与外链安全规则，长链接必须允许换行，鼠标焦点不能留下默认黑框，键盘 `focus-visible` 提示必须保留。
