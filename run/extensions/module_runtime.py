...
from run.config import ConfigError, dotenv_values
from run.infra import cancellable_subprocess_kwargs, terminate_process_tree
...
# Directory-owned modules are executable code, but they must not implicitly
# inherit Provider/Web credentials from the long-lived framework process.  A
# deliberately small runtime allowlist keeps Python and ordinary subprocesses
# usable on Windows, macOS and Linux.  Module-specific values belong in the
# module's ignored ``.env`` file and are overlaid below.
_MODULE_BASE_ENV_NAMES = frozenset(
    {
        "APPDATA",
        "COMSPEC",
        "CONDA_DEFAULT_ENV",
...
_MODULE_TRUSTED_RUNTIME_ENV_NAMES = frozenset({"KEMO_AGENT_WEB_BASE_URL"})
_MODULE_ENV_MAX_BYTES = 256 * 1024
_MODULE_ENV_MAX_ENTRIES = 512
_MODULE_ENV_MAX_VALUE_CHARS = 64 * 1024
...
def module_subprocess_environment(module_root: Path) -> dict[str, str]:
    """Build an isolated environment for one Expand/Sense module process.

    The returned mapping is independent for every invocation.  Framework
    credentials from ``os.environ`` are therefore not visible to directory
    modules, while an optional module-local ``.env`` remains the explicit
    credential/configuration source.  The local file is ignored by Git and is
    never installed into the parent process.
    """

    resolved_root = module_root.resolve()
    local_env = resolved_root / ".env"
    if _is_link(local_env):
        raise ModuleRuntimeError("模块 .env 不能是符号链接或目录联接")

    source = dict(os.environ)
    if os.name == "nt":
        source_by_name = {name.upper(): value for name, value in source.items()}
        environment = {
            name: source_by_name[name]
            for name in _MODULE_BASE_ENV_NAMES
        ...
        environment = {
            ...
            if name in _MODULE_BASE_ENV_NAMES or name.startswith("LC_")
        }

    try:
        local_values = dotenv_values(
            local_env,
            max_bytes=_MODULE_ENV_MAX_BYTES,
            max_entries=_MODULE_ENV_MAX_ENTRIES,
            max_value_chars=_MODULE_ENV_MAX_VALUE_CHARS,
            duplicate_policy="error",
            case_insensitive_names=os.name == "nt",
        )
    except (ConfigError, ValueError) as exc:
        raise ModuleRuntimeError(f"模块 .env 无效：{exc}") from exc

    if os.name == "nt":
        environment.update(
            {name.upper(): value for name, value in local_values.items()}
        )
        trusted_source = {name.upper(): value for name, value in source.items()}
    else:
        environment.update(local_values)
        trusted_source = source
    # The Web endpoint is generated by the running framework after dynamic
    # port selection.  It must win over a stale module-local value so kemo_app
    # keeps following the actual local backend.
    for name in _MODULE_TRUSTED_RUNTIME_ENV_NAMES:
        value = trusted_source.get(name)
        ...
        try:
            process = subprocess.Popen(
                ...
                env=(
                    ...
                    else module_subprocess_environment(cwd)
                ),
