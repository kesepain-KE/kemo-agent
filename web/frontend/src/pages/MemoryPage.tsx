...
import { Brain, CalendarDays, ChevronLeft, ChevronRight, Eye, FileText, LoaderCircle, Plus, RefreshCw, RotateCcw, Save, Search, Trash2, X } from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import { useOutletContext } from 'react-router-dom'
import { deleteImportantMemory, deleteMemory, getImportantMemory, getMemoryItem, getMemorySummary, putMemory, updateImportantMemory } from '../api/client'
...
export function MemoryPage() {
  ...
  useEffect(() => {
    if (detailContent !== undefined && selectedKey) {
      setDraft(detailContent)
      setOriginal(detailContent)
    }
  }, [detailContent, selectedKey])
  ...
  return <ModuleFrame kicker="Memory & Lifecycle" title="记忆" description="管理智能体不同时间尺度的记忆片段，让 kemo-agent 在对话中持续学习并提供更贴合的帮助。" actions={<div className={styles.headerActions}>
    <button type="button" className="module-btn" onClick={reload} disabled={summary.isFetching || important.isFetching}><RefreshCw size={15} className={summary.isFetching ? 'spin' : ''} />重新读取</button>
    ...
    <div className={styles.workspace}>
      <section className={styles.listPanel}><div className={styles.panelHeading}><div><strong>{TIER_LABELS[activeTier]}</strong><span>{WEIGHTED_TIERS.has(activeTier) ? '编辑后权重 +1，每天最多增加一次。' : activeTier === 'important' ? '独立 Markdown 文件，可直接编辑完整内容。' : '手动创建并长期保存的稳定记忆。'}</span></div><span className={styles.panelCount}>{visibleRows.length}</span></div><div className={styles.toolbar}><label className={styles.search}><Search size={15} /><input value={queryText} placeholder="搜索当前记忆栏……" onChange={(event) => setQueryText(event.target.value)} /></label><select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="newest">按更新时间</option><option value="oldest">按最早时间</option>{WEIGHTED_TIERS.has(activeTier) && <option value="weight">按权重排序</option>}</select></div><div className={styles.rows}>{pagedRows.length ? pagedRows.map((row) => <article key={row.key} className={`${styles.row} ${selectedKey === row.key ? styles.rowSelected : ''}`} onClick={() => setSelectedKey(row.key)}><span className={styles.rowDot} /><div className={styles.rowCopy}><strong>{row.title}</strong><span>{row.preview || row.filename}</span></div><div className={styles.rowMeta}>{WEIGHTED_TIERS.has(row.tier) && <span className={styles.weight}>权重 {row.weight}</span>}<small>{formatUpdated(row.updatedAt)}</small><button type="button" onClick={(event) => { event.stopPropagation(); setSelectedKey(row.key) }}>编辑</button><button type="button" onClick={(event) => { event.stopPropagation(); setPendingDelete(row) }}>删除</button></div></article>) : <EmptyPanel title="当前记忆栏暂无内容" description="可以切换其他记忆栏或重新读取数据。" icon={<Brain size={22} />} />}</div><div className={styles.pagination}><span>共 {visibleRows.length} 条</span><div><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={14} /></button><b>{page} / {totalPages}</b><button type="button" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}><ChevronRight size={14} /></button><select value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))}>{PAGE_SIZES.map((size) => <option key={size} value={size}>{size} 条/页</option>)}</select></div></div></section>
      <aside className={styles.editorPanel}><div className={styles.editorHeading}><div><strong>编辑查看</strong><span>{selected ? selected.title : '选择一条记忆'}</span></div>{selected && <button type="button" aria-label="关闭编辑查看" onClick={() => setSelectedKey('')}><X size={16} /></button>}</div>{!selected ? <div className={styles.editorEmpty}><Brain size={42} /><strong>选择一条记忆</strong><span>点击左侧记忆后在此处查看和编辑。</span></div> : <><div className={styles.metadata}><span>记忆类型：<b>{TIER_LABELS[selected.tier]}</b></span><span>更新时间：<b>{formatUpdated(selected.updatedAt)}</b></span>{WEIGHTED_TIERS.has(selected.tier) && <span>当前权重：<b>{item.data?.weight ?? selected.weight}</b></span>}</div>{WEIGHTED_TIERS.has(selected.tier) && <div className={styles.weightNotice}><Brain size={14} />{weightToday ? '该记忆今天已经因编辑增加过权重，再次保存不会继续增加。' : '保存编辑后权重 +1，同一条记忆每天最多增加一次。'}</div>}<div className={styles.editorToolbar}><button type="button" className={previewing ? styles.previewActive : ''} onClick={() => setPreviewing((value) => !value)}>{previewing ? <><FileText size={14} />返回编辑</> : <><Eye size={14} />Markdown 预览</>}</button>{changed && <small>有未保存修改</small>}</div><div className={styles.editorBody}>{loadingDetail ? <div className={styles.editorLoading}>正在读取记忆内容…</div> : previewing ? <article className={styles.markdown}><ReactMarkdown remarkPlugins={[remarkGfm]}>{draft}</ReactMarkdown></article> : <textarea value={draft} spellCheck={false} placeholder="输入记忆内容……" onChange={(event) => setDraft(event.target.value)} />}</div><div className={styles.editorActions}><button type="button" className="module-btn" disabled={!changed || save.isPending} onClick={() => setDraft(original)}><RotateCcw size={14} />恢复</button><button type="button" className="module-btn danger" disabled={save.isPending || remove.isPending} onClick={() => setPendingDelete(selected)}><Trash2 size={14} />删除此记忆</button><button type="button" className="module-btn primary" disabled={!changed || save.isPending} onClick={() => save.mutate()}>{save.isPending ? <LoaderCircle size={14} className="spin" /> : <Save size={14} />}保存编辑</button></div>{save.isError && <ModuleError message={String(save.error)} />}</>}</aside>
    </div>
