...
> 本文保留 1.2.2 的历史检查记录。后续的工具参数终态校验、诊断递归限制和后台进程安全修复以 `release-1.2.3-stability.md` 为准。
...
4. 运行结束音效只有真正成功结束才播放；运行失败音效只在明确的最终 `failed/error` 终态播放。
   取消、暂停、受限停止和长任务中间 Run 都不播放。两类音效按用户分别保存，只在
   Windows 桌面网页端启用；浏览器不支持播放时才使用受保护的终端降级。
