...
_RETRY_SAFE_OPERATIONS = frozenset(
    {
        ("global", "kemo_graph", "query"),
        ("global", "kemo_graph", "status"),
    }
)
...
from run.extensions.expand_errors import (
    ExpandOperationError,
    ExpandRuntimeError,
    _expand_operation_error,
        ...
        with module_execution_lock(
            ...
            if isinstance(result, dict):
                ...
                if result.get("ok") is False or status in {"error", "failed", "failure"}:
                    raise _expand_operation_error(
                        _operation_failure_reason(result),
                        scope=scope,
                        module=module,
                        command=command,
                        metadata=result,
