...
- 记忆碎片按 A / B 两类区分粒度：画像与特征类允许在同一维度内合并更新，事实与规则类保持最小碎片；判据是「拆开后单条还能不能独立理解」。提取条数改按独立事实计数，不再按对话轮数折算，单轮上限提升。
...
- CLI 退出与 Cron 任务收尾时补齐会话关闭与记忆提取登记，此前只有 Web、App 与外部消息三条链路会关闭会话，CLI 与 Cron 的对话会永久悬挂而不进入记忆。
