...
不立即调用引导 API；Shift+Enter 保留换行，中文输入法组合输入时不发送。输入框还提供“下一轮发送”
快捷入口，同样加入该队列。当前 Run 收束后，队列按展示顺序逐条作为后续用户轮次发送。
空闲时仍使用正常“发送”，不改变首轮交互。
...
CLI 与 Cron 主智能体在入口收束时统一执行“记忆提取登记 → 逻辑会话关闭”，RuntimeHost 每小时以
`session_lifecycle_sweep` 扫描超过配置阈值的闲置会话。扫描尊重会话锁、运行态和 Web lease，单轮最多
关闭 50 个且不删除历史；这些兜底动作均为 best-effort，不能覆盖原始请求或 Cron 状态。
...
页面每 15 秒发送会话租约心跳，恢复显示或重新联网时补发，pagehide 与组件卸载时尽力释放页面关联。
空会话离线清理后旧页恢复收到 404，会解除绑定并解释原因，不把迟到心跳当作重建会话请求。
被清理的旧 `session` URL 不会自动创建替代对话；只有用户显式进入 `new_session=1` 链接、执行正常
新建/首次发送操作或调用会话创建 API，才创建新的随机空间。Web 启动 90 秒后还会执行一次旧空间巡检：离线且有内容的空间关闭活跃绑定
