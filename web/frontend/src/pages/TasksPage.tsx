...
function PlanDetailPanel({ plan }: { plan?: PlanSummary }) {
  return <aside className={`${styles.detailPanel} ${themeStyles.surface} ${themeStyles.detail} ${!plan ? themeStyles.emptyDetailPanel : ''}`}><header><div><h2>计划查看</h2><p>可查看未运行 / 已完成计划</p></div></header>{!plan ? <div className={`${styles.detailEmpty} ${themeStyles.expandedEmpty}`}><ClipboardList size={25} /><strong>选择一个计划</strong><span>点击未运行或已完成的计划查看详情。</span></div> : <div className={styles.detailBody}><div className={styles.detailTitle}><span className={styles.planAvatar}>P</span><strong>{plan.title}</strong><StatusChip status={plan.status} /></div><dl><div><dt>计划 ID</dt><dd>{plan.plan_id}</dd></div><div><dt>状态</dt><dd><StatusChip status={plan.status} /></dd></div><div><dt>步骤总数</dt><dd>{plan.steps.length}</dd></div><div><dt>创建时间</dt><dd>{formatDateTime(plan.created_at)}</dd></div><div><dt>更新时间</dt><dd>{formatDateTime(plan.updated_at)}</dd></div></dl><section><h3>计划描述</h3><p>{plan.description || '暂无描述'}</p></section><section><h3>步骤概览</h3><div className={styles.detailSteps}>{plan.steps.map((step, index) => <div key={step.step_id}><b>{index + 1}</b><span>{step.title}</span>{step.status === 'completed' && <Check size={14} />}</div>)}</div></section></div>}</aside>
}
