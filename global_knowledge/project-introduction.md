...
`1.2.2` 是稳定性和维护更新。`run/` 按职责拆成领域包，并删除旧的平铺导入路径；启动器恢复正确项目根目录，并在默认端口不可用时使用备用端口；本机 `kemo_app` 桥接跟随启动器发布的实际 Web 地址。任务计划支持修改、重试、重置、修订查看和安全回滚，持久化前会脱敏明显凭据形态。网页结束音效按用户独立保存，只在 Windows 桌面网页端显示和播放；发送附件后立即清除引用；运行中引导上传使用 `purpose=input`。本次同时补充项目路径、包结构、备用端口和模板合同测试。旧 `run.*` 平铺导入路径不再支持，外部插件需要迁移。`kemo_app` 桥接端口跟随修复记为 `1.1.5`。
