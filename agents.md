...
> 当前稳定版本：`kemo-agent 1.2.6`，配套 Kemo 网关为 `kemo-adapter-api 0.7.6`。本版本重点收紧模块子进程环境、跨平台 Shell、Provider 诊断和 Kemo Graph 文件同步边界；模块密钥不得从框架根环境隐式继承，也不得写入浏览器、Prompt、历史或日志。遇到旧文档与本段冲突时，以当前代码和本段的安全规则为准。
