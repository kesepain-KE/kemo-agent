...
Web“用户配置”组件专用命令，只设置 `scheme`、`gateway_ip` 和 `gateway_port`。未配置独立
`STATUS_TOKEN` 时先保存待激活地址；已经激活时保留现有 Token 和采集参数，验证新地址成功后
才替换配置。验证失败保留旧配置并记录脱敏离线状态。该命令不开放任何网关管理写能力。
