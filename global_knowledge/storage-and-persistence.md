...
`GET /api/users/{user}/runtime/logs` 独立提供近期日志，不重新聚合 Prompt、记忆、Token 和整页运行状态。参数为 `category=all|backend|threads|terminal|message`、`page`（从 1 开始）、`page_size`（默认 25，上限 100）、`refresh`（默认 false，true 强制绕过读取缓存）。返回 entries、counts、分页、生成时间、cache 和 source_errors；页码超过当前记录窗口时回到最后一页。

- **全部**：合并以下来源的有界近期视图，不是全量历史日志。各分类数量也仅代表此窗口。
- **后端日志**：当前用户和 `__system__` 的 Cron 持久化记录，以及当前进程观察到的工具调用与 RuntimeHost 组件生命周期摘要；不是原始 Python/服务器文本日志。高频成功任务仍可能以聚合记录出现。
- **后端线程**：当前后端进程最多 200 个线程的即时快照，仅含线程 ID、存活/守护状态和通用类别，不保存轮询快照，不展示自定义线程名、栈和局部变量，也不声称是历史执行轨迹。
- **终端日志**：当前进程 `execute_tool` 观察到的 shell 调用状态、耗时、退出码和错误类型；不复制命令、参数、环境变量、输入输出或异常正文。完整输出继续留在原工具卡片与历史。后台提交显示运行中而非完成；独立后台进程的完成状态需通过后续状态调用被观察到，不保证实时追踪所有系统终端。
...
网页分类、翻页和刷新使用独立查询，前台每 10 秒刷新，后台页面不定时轮询；切换用户不沿用旧用户展示数据。刷新日志强制绕过服务端 TTL，不触发整页聚合。
