...
能力选择、发现摘要、来源路径和数据/指令边界的统一规范见 `prompt-authoring-standard.md`。本页只维护各类型的实际清单与运行合同，不要求固定内部工程结构。

## 拓展模块创建文档

拓展（Expand）是可持续使用的外部设备、API、服务或配套工程连接。它可独立开启“状态数据进入 Prompt”和“由智能体发起调用”，不要求同时具备两者。纯可复用方法使用技能；全局共享的只读观测优先感知，需要用户隔离或专用服务连接的只读模块也可使用拓展；独立 LLM 推理使用子智能体。
...
注入层可以进入主智能体 Prompt，并附真实操作说明路径；操作层不会自动全部注入，应由智能体按需读取。Prompt 管线按总注入闸门与实时开关决定省略、轮内固定或请求级重读 `input_data.md`；cron 只负责刷新文件，不负责模板变量替换或直接注入。重读文件不等于重新采集设备。
...
技能（Skill）是为一类任务提供工作方法、判断分支和验收标准的可复用指令包。主智能体自动注入标题和发现摘要（附作用域、来源路径），匹配任务后按需读取正文和资源，不会自动加载全文或执行脚本。技能不会注册 Provider function call；真正可执行的工具只能放在 `plugins/`。
...
运行时递归寻找技能目录中的 `SKILL.md`；这是唯一必需的框架合同。文件必须包含一级标题，一级标题之后、下一个二级标题或 `---` 之前的内容作为技能描述。标题前可以有注释或示例；围栏代码和四空格缩进代码中的 `#` 不参与标题或 `## Tool` 识别。除此之外，框架不规定章节数量、名称、写作方式或目录结构。
...
拓展从模块目录自己的 `.env` 或其他私有配置读取，不能依赖框架根目录的 Provider/Web 环境，也不能写入桥接清单、Prompt、回复或日志。模块子进程只继承 PATH、临时目录、证书路径等运行必需环境；运行时生成的本机 Web 地址由框架单独传入。模块 `.env` 中不要重复定义同名变量；损坏、重复或过大的文件只会让该模块本次运行失败，不会改变框架进程环境。
...
- 远程 URL、Token、密码、Cookie 和私钥只从拓展目录自己的 `.env` 或私有配置读取，不写入
