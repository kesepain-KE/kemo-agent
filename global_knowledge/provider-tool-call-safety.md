...
`invalid_tool_arguments` 后，主运行时可以建立新的逻辑 Provider 请求，临时在系统消息中加入
“只重新生成完整工具调用”的纠错指令。该恢复不是传输重试，也不会复用失败请求的
`request_id`。
...
恢复成功后只执行新响应中的完整调用；失败尝试不会进入对话正文或工具卡片，但已经产生的
Provider Usage 仍会计入本轮统计。若已经存在可见输出、完整调用或达到上限，则保持原有明确
失败终态，不能静默重放可能有副作用的工作。
