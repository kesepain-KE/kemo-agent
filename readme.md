...
这是一次面向长期智能、会话生命周期、模块扩展能力与 Web 交互一致性的正式版本更新。

- 潮汐记忆按“能否独立更新、失效、加权和检索”拆分事实粒度；用户历史加权绑定原轮次、上海自然日和检索引用，创建、强化、修订、日锁与幂等回执形成可审计证据链。晋升超限继续在事务内拆分，技能创建、修改和升级会提醒是否融合相关记忆，默认保留原记忆。
- Web、App、CLI、外部消息和 Cron 的对话续接、租约、关闭与离线转记忆进一步统一；closed 入队补偿、空闲会话巡检、删除栅栏和有界 runtime 缓存共同避免会话假复活、长期悬挂和平方级写盘。
...
这是一次面向记忆进化、会话生命周期与写盘可靠性的更新。

- 记忆碎片按 A / B 两类区分粒度：碎片数量以独立事实为准，依次检查能否独立更新、失效、加权和检索；画像类只有同一稳定子主题且四个边界完全一致时才能形成特征簇，事实与规则类保持最小碎片。字符上限只用于防止单片失控，提取候选不再按对话轮数乘系数裁剪。
- 晋升到新档位时若超过该档位粒度上限，会在同一事务内拆成多条子碎片：子碎片继承原时效、权重归零、不产生加权事件，任一条无效则整批回滚保留源碎片；碎片融合仅在「同一事实的更新版本」之间发生，融合结果永不跨越上限以避免反复拆分与融合。
- 用户主动创建或修改技能时，若存在相关记忆会先提示是否融合，默认保留记忆不删除；后台自动创建技能不打扰。
- CLI 改用独立会话绑定，退出与 Cron 任务收尾时补齐会话关闭与记忆提取登记；Web/App 使用跨进程存活租约，离线有数据会话关闭后进入记忆队列，closed 入队失败会由兜底巡检补偿。
- 新增空闲会话兜底扫描系统任务，按小时关闭超过一天的未关闭会话；运行中、排队中、持锁中、持有 Web 租约的会话一律跳过，始终先入队记忆再关闭。
- runtime 工作区改为进程内缓存并设容量与淘汰上限，每轮提交只写 archive；进程缓存缺失时从 archive 尾部按需重建，跨进程写入通过版本栅栏自动失效。长会话不再因逐轮重写完整正文而产生平方级写盘。
- 裁剪窗口时识别「旧尾部即新前缀」的位移场景，只删除移出的头部并追加新尾部，无法安全识别时回退整表重建。
- 修复 Web 拓展与感知页「注入预览」的片段错位：预览此前自行拼装片段并用累加游标切片，比真实片段少了框架头部行，导致第二块起逐块渲染出前一块的尾部；现在片段位置由权威侧给出，预览只按标识取值。
- 运行日志面板改为卡片式布局并在宽屏下扩展可用宽度，终端日志改为摘要卡片加只读控制台并补响应式布局；终端日志条目区分标准输出与标准错误并增加载入窗口、条目计数等摘要，同时保持脱敏。
...
- 主智能体与子代理的输出解析错误支持最多 5 次有界重试；上下文压缩的记忆提取结果在重试中复用，避免重复写盘；`context_manage` 专用修复失败不会被外层重复放大。
- 持久化 Provider 响应标识会脱敏并限制长度，减少异常响应对历史大小和敏感信息边界的影响。

### 1.2.2 更新

这是一次稳定性和维护更新。

- `run/` 已按职责拆成多个目录，旧的平铺导入路径不再支持。
...
`kemo_app` 桥接版本为 `1.1.5`。旧插件如果仍导入 `run.agent_runner`、`run.task_plan_store` 等路径，需要改用新的 `run.<领域>` 入口。
