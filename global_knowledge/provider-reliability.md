...
主对话运行（L2）和子代理运行（L4）共用 `run/retry/` 的尝试级策略。一次中断事件最多执行 **5 次重试**，即首轮加 5 次重试共 **6 次尝试**；`retrying` 事件中的 `max_attempts` 因此为 6。每次尝试都会创建新的逻辑 Provider 请求和新的 `request_id`，可能产生新的模型计费，不属于同一请求的网络续传。

重试额度只属于当前 Run 内当前连续失败序列，不写入会话级持久计数器。一次尝试成功后该序列结束；同一对话稍后再次出现问题，或者用户在失败落盘后点击“继续”开启新 Run，都会重新获得完整的 5 次重试额度。因此同一对话出现两次独立 API 异常时，两次异常理论上分别最多重试 5 次，而不是共同分享 5 次。

除以下边界外，错误声明为 `retryable=false`、HTTP 400/401/403/404/409/422、鉴权错误、请求校验错误、协议错误、上下文压缩恢复耗尽、工具参数修复耗尽和普通运行时异常都必须消耗完整运行级预算后才停止：

- 用户取消：尊重用户意志，不重试，立即落盘；
- `completed` 和工具轮数上限产生的 `limited`：属于正常或受控结束，不重试；
- Chat 传输层严格的 2 次预算已经耗尽并显式设置 `retry_budget_exhausted=true`：不再叠加外层 6 次尝试；
- `KeyboardInterrupt` / `GeneratorExit` 等宿主进程控制边界继续向外传播。

工具参数修复和上下文压缩属于尝试内部的修复型重发（L3），各自预算不计入上述 5 次；只有其自身预算耗尽后，最终失败才进入 L2/L4 的 6 次尝试序列。成功工具结果的跨尝试重放仍受 32 次、120,000 字符的上限和副作用安全规则约束。
...
- 重试预算耗尽后错误被标记 `retryable=False`、`retryable_declared=True` 和 `retry_budget_exhausted=True`，这是 Chat 传输层阻止外层运行级重试的唯一终局声明。
