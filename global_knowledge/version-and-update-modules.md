...
组件版本不要求始终与根版本相同。仅有部分板块发生变化时，只推进对应组件版本。`0.9.2` 修复子代理多轮工具循环中的 reasoning ID 冲突，并为历史归档、临时上下文、摘要缓存和历史索引统一增加 Windows 短暂文件锁重试，因此只推进 core；agents 保持 `0.8.0`，plugins 保持 `0.8.1`，web 保持 `0.9.0`。
