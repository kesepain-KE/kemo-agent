...
- 临时重要记忆作为可重建热画像独立维护，只由临时三层单向提炼，不会反向加权源记忆；5000 字符是 Prompt 注入预算，合理的较长正文可完整落盘，20000 字符才是输出防失控上限；
