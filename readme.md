...
- [使用方式](#使用方式)
- [架构概览](#架构概览)
- [Kemo Tidal Engram](#kemo-tidal-engram)
- [上下文管理](#上下文管理)
- [子代理体系](#子代理体系)
- [资源分层](#资源分层)
- [消息路由](#消息路由)
...
- **CLI**：本地交互式命令行
- **Message Router**：面向所有外部消息平台的统一路由
- **Cron**：持久化定时任务与后台自动执行
...
### Kemo Tidal Engram 潮汐式生命周期记忆

Kemo Tidal Engram 是 kemo-agent 内置的潮汐式生命周期记忆碎片系统。它通过记忆衰减、权重演化、层级迁移、重要性晋升与上下文召回，维持智能体长期运行中的认知连续性。

记忆如同潮汐——日常对话中浮现的信息会被自然筛选，重要的内容随时间的浪涌逐步沉淀为长期认知，而不再被提及的内容则会自然地淡出。这不是对对话的无差别记录，而是一种模拟生物记忆节律的认知维持机制。

系统在后台持续运行记忆审阅与碎片整理，确保智能体在数天、数月乃至更长时间跨度中始终能够回忆起真正重要的人和事。
...
| 消息路由 | 统一 Transport 抽象层，架构适配所有外部消息平台 |
| Web 管理端 | 完整浏览器端管理界面，覆盖对话与运维全维度 |
...
Web 前端提供完整的系统管理能力，覆盖对话交互、记忆与知识管理、任务调度及运行时可观测性。
...
│   ├── memory.py           # Kemo Tidal Engram 潮汐记忆引擎
│   ├── memory_pipeline.py  # Engram 碎片提取管线
│   ├── memory_migrate.py   # Engram 格式迁移工具
│   ├── tools.py            # 工具发现、执行与去重
...
→ 执行 Kemo Tidal Engram 记忆审核
→ 构建 17 段 PromptBundle
→ 选择上下文并执行预算压缩
→ 进入模型与工具循环
→ 成功：提交历史、更新潮汐权重、提取 Engram 碎片
→ 失败或取消：不写入历史，不更新记忆
```
...
Kemo Tidal Engram 按潮汐节律将记忆分布在不同时间层中。短暂停留的信息被归入浅层潮汐带；被持续提及和引用的内容则随时间逐步向更深层的潮汐带迁移；超过一定时间仍未被再次触发的碎片，则会自然消逝。

这种设计使智能体的认知始终偏向于实际使用中反复加固的记忆，而非不加区分地保留一切过往对话。潮汐的涨落节奏、各层间的迁移条件以及每日的加权频次，均由系统内部自动管理，无需用户手动干预。

后台的 Engram 审阅进程会定期巡检各层的记忆状态，将符合条件的碎片推进到下一潮汐层，并维护一份"重要 Engram 画像"——它汇聚了当前最活跃、最具时效性的记忆片段，确保在与用户对话时，智能体始终能想起最相关的信息。

所有记忆以可迁移的开放格式保存在用户目录中，支持备份、版本管理和跨实例迁移。
...
| `memory_temporary_important` | Cron / 主智能体 | 巡检潮汐 Engram 并维护重要 Engram 画像 |
| `self_improve` | 压缩、晋升、手动审阅 | 提取 Engram 碎片、潮汐权重演化与层间迁移 |
...
外部消息路由采用统一的 Transport 抽象层设计，从架构上适配所有外部消息平台，而非绑定某一特定协议。

消息通过标准化的身份映射进入智能体运行时，与 Web 和 CLI 入口共享同一套上下文隔离、记忆体系与能力准入策略。每条外部消息均经过幂等去重，确保同一事件不会被重复处理；附件与文本以统一 `RunEvent` 协议聚合，出站回复自动分发回对应平台。

平台接入细节对上层完全透明——无论消息来自何处，智能体始终以一致的方式感知和响应。
...
kemo-agent 内置完整的 Web 管理界面，覆盖对话交互与系统运维的各个维度。

前端基于 React + TypeScript + Vite 构建，后端通过 FastAPI 提供 RESTful 与 SSE 流式接口。所有运行时可观测数据——从上下文状态、Engram 记忆、知识索引到 Provider 负载——均可在浏览器中直接查看与管理，无需登录服务器或编辑配置文件。
