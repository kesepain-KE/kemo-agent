...
2. 普通对话请求，或归属当前来源/会话、状态仍为 `running` 的 `agent_managed` 前台任务计划执行请求；
...
Web/App 已批准计划的连续执行复用会话长任务开关。达到工具调用上限时，中间 Run 的结束不关闭
整个计划流，也不把计划改成 `paused`；后续 Run 保留 `_task_plan_id`、`_task_plan_mode`、来源和
会话身份，在原执行流内继续，不重新领取、批准或重建计划。

续跑提示要求读取最新计划及上一轮结果，遵守步骤依赖，并继续通过 `step_done` / `step_fail`
写回结果。已成功操作不得重做；操作成功但尚未写回步骤结果时，先核对并补记。
每次判定续跑及交接后发起下一 Run 前均重读计划状态；已暂停、取消、失败、完成、不可读取或不属于
当前会话的计划不得续跑。计划创建/等待批准仍是独立终止边界，不能用长任务开关绕过。

关闭长任务开关后，当前 Run 自然收束，仍未完成的前台计划由原有执行流清理逻辑暂停；取消、
Provider 错误和 128 个 Run 硬上限继续生效。后台逐步执行器不接入这一前台跨 Run 编排。
