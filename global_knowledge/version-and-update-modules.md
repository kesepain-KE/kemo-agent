...
组件版本不要求在后续版本中始终与根版本相同；仅有部分板块发生变化时，可以只推进对应组件版本。`1.0.0` 是主生态首次完整发布，因此根版本与 core、agents、plugins、web 四个组件统一进入 `1.0.0`。`1.0.1` 是主生态补齐后的系统性稳定性复核版本，本轮四个组件同步推进，表示检查覆盖完整框架而不只是单一板块。
