...
class UpdateModuleTests(unittest.TestCase):
    ...
    def test_failed_backup_does_not_leave_partial_backup_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                backup_update.make_backup(root=root, dry_run=False)
    ...
    def test_runtime_state_initialization_creates_current_databases(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            migrations_update.initialize_runtime_state_databases(root=root, dry_run=False)
    ...
    def test_build_requires_npm_even_when_stale_dist_exists(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.object(build_update, "_resolve_npm_command", return_value=None),
                self.assertRaises(UpdateError),
            ):
                build_update.build_web_frontend(root=root, dry_run=False)
    ...
    def test_core_preserves_message_out_and_only_updates_register_files(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            write_json(source / "config" / "global_config.json", {"schema_version": 1, "new": True})
            ...
            self.assertEqual(json.loads((target / "config" / "global_config.json").read_text(encoding="utf-8"))["schema_version"], 1)
    ...
    def test_dispatcher_parses_module_and_component_versions(self) -> None:
        ...
        self.assertEqual(manifest_update.version_for_module(document, "all"), "1.2.0")
        self.assertEqual(manifest_update.version_for_module(document, "plugins"), "1.3.0")
    ...
    def test_dispatcher_rejects_incomplete_version_manifest(self) -> None:
        ...
        with self.assertRaises(UpdateError):
            manifest_update.validate_version_document(document, "远程")
    ...
    def test_dispatcher_loads_update_board_from_cloned_source(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            results = dispatcher_update.run_modules(
                ["core"],
                source,
                target,
                dry_run=False,
    ...
    def test_finalize_version_writes_only_after_explicit_commit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest_update.finalize_version_document(
                    local,
                    remote,
                    "plugins",
                    dry_run=False,
    ...
    def test_plugins_only_dispatch_does_not_run_core_migrations_or_builds(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with (
                mock.patch.object(cli_update, "_print_platform_warning"),
                mock.patch.object(cli_update, "load_local_version_document", return_value=version),
                mock.patch.object(cli_update, "load_remote_version_document", return_value=version),
                mock.patch.object(cli_update, "should_update", return_value=True),
                mock.patch.object(cli_update, "require_commands"),
                mock.patch.object(cli_update, "clone_latest", return_value=ROOT),
                mock.patch.object(cli_update, "load_source_version_document", return_value=version),
                mock.patch.object(cli_update, "source_revision", return_value="test"),
                mock.patch.object(cli_update, "run_modules", return_value=[{"module": "plugins", "status": "ok", "details": [], "warnings": []}]) as run_modules,
                mock.patch.object(cli_update, "migrate_user_skeletons") as migrate_skeletons,
                mock.patch.object(cli_update, "initialize_user_memory_databases") as initialize_memories,
                mock.patch.object(cli_update, "initialize_runtime_state_databases") as initialize_runtime_state,
                mock.patch.object(cli_update, "build_web_frontend") as build_web,
                mock.patch.object(cli_update, "refresh_dependencies") as refresh_dependencies,
                mock.patch.object(cli_update, "finalize_version_document") as finalize_version,
            ):
                result = cli_update.main(["--module", "plugins", "--yes"], root=root)
    ...
    def test_failed_web_build_does_not_commit_version(self) -> None:
        ...
        local = {
            "version": "1.0.0",
            "components": {name: {"version": "1.0.0"} for name in MODULES},
        }
        remote = {
            "version": "2.0.0",
            "components": {name: {"version": "2.0.0"} for name in MODULES},
        }
        results = [
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.object(cli_update, "_print_platform_warning"),
                mock.patch.object(cli_update, "load_local_version_document", return_value=local),
                mock.patch.object(cli_update, "load_remote_version_document", return_value=remote),
                mock.patch.object(cli_update, "should_update", return_value=True),
                mock.patch.object(cli_update, "require_commands"),
                mock.patch.object(cli_update, "clone_latest", return_value=ROOT),
                mock.patch.object(cli_update, "load_source_version_document", return_value=remote),
                mock.patch.object(cli_update, "source_revision", return_value="test"),
                mock.patch.object(cli_update, "run_modules", return_value=results),
                mock.patch.object(cli_update, "migrate_user_skeletons"),
                mock.patch.object(cli_update, "initialize_user_memory_databases"),
                mock.patch.object(cli_update, "initialize_runtime_state_databases"),
                mock.patch.object(
                    cli_update,
                    "build_web_frontend",
                ...
                result = cli_update.main(["--module", "all", "--yes"], root=root)
