...
当前稳定版本为 `1.3.1`（延续 1.3.0 能力与网关兼容基线），已确认兼容 `kemo-adapter-api 0.8.2`，两端使用的 Kemo 1.0 线路协议匹配。版本号、各组件版本和兼容网关基线以根目录 `version.json` 为准。本文只说明项目定位、当前能力、部署方式和功能边界，不保存历史版本或发布记录；具体实现合同请阅读对应专题文档。
...
删除会话会同时清理归档、运行时窗口、正文、活跃绑定和后台副本，并在历史库中留下很小的删除栅栏。
如果旧 Run 在删除之后才结束，它的迟到提交会被丢弃，不会让已删除会话重新出现。知识库索引同样会
拒绝符号链接和 Windows junction，避免把项目目录外的文件带入 Prompt。
...
Kemo 协议是第一优先级的原生链路，提供 Asset API、SSE 续传、统一 Usage 和幂等取消。Chat
Completions 兼容模式面向第三方 OpenAI 兼容服务，按"最小兼容传输"实现：请求净化（不注入
非标准字段）、流式工具调用宽容聚合（重复 id/name 幂等、宽松 index、完整 JSON 对象参数）、
截断调用一律不可执行、有界输出前网络恢复（零输出才重试，2 次预算）、工具不支持时一次性
降级。两种模式在一次 Run 内固定，不自动互相回退。细节见 `provider-reliability.md`。
...
App 桥接在重启恢复时可以按设备 ID 查询该设备的全部活动 Run，再按明确的 `session_id` 读取快照和
继续事件流；没有设备或会话范围的查询会被拒绝，避免把一个用户的所有运行无范围暴露给客户端。
...
当前 1.3.1 已发布；下列一键命令可直接使用。原生安装依赖 Release 的
...
用户名和密码必须同时配置或同时留空。详细字段、优先级和安全规则见 `configuration-reference.md`。
...
- `kemo-adapter-api 0.8.2`：Kemo 模型网关与 Provider 适配项目；1.3.1 延续 1.3.0 已确认的协议匹配；
