...
热创建、列出并校验当前用户拓展或共享拓展。工具原子建立框架可发现的根合同和声明式用户配置组件；模块内部实现和工程结构不受模板限制，创建后立即使用真实 Expand 与组件面板契约复验。

适用于可持续使用的设备、服务或配套工程连接；数据注入和调用能力可独立开启，允许只读拓展。纯共享环境观测优先感知，纯任务方法用技能。创建前确认范围与副作用并查重；编写规范见 `global_knowledge/prompt-authoring-standard.md`。
...
- 需要具有用户隔离、共享范围控制或服务专用生命周期的周期性数据连接；纯全局共享只读观测优先感知。
...
5. 创建成功后报告路径、根合同、`module/` 组件文件和运行时校验结果。默认面板只提供已经真实实现的立即采集能力；随后按模块真实配置与命令调整面板，不得声明尚未实现的按钮，也不得把所有功能强行塞进入口脚本。

## 模块目录不是固定模板

`expand_creater` 建立的是框架最小合同和可删除/可改写的组件骨架，不是完整工程脚手架。拓展目录内部可以包含任意文件、任意层级目录、已有项目源码、资源、配置和构建产物；未知内部文件不会自动注册、注入或执行，也不应仅因模板没有列出而被拒绝。保持原项目结构或采用极小实现都可以，唯一要求是清单声明的入口最终适配框架协议并留在模块作用域内。

## 已有拓展操作

- `list`：列出一个 scope 的模块、功能说明、输入健康状态和结构校验结果。
- `validate`：校验指定模块的清单字段、引用文件、Python 语法、操控文档结构、路径边界与真实运行时兼容性；存在 `module/panel.json` 时同时用框架解析器校验组件合同。

## 安全约束

- `create` 不覆盖同名目录，目录级临时写入完成后才发布；创建后校验失败会回滚整个新模块。
- 模块目录和清单引用文件不得是符号链接或目录联接，也不得跳出目标 scope。
- 说明、Markdown 和自定义 Python 中不得写入疑似 API Key、Token、密码或其他敏感凭据；代码应从模块目录自己的 `.env` 读取密钥。模块子进程不会隐式继承框架根目录的 Provider/Web 凭据。
- 根目录 `template/expand/` 是创建器的唯一默认模板来源，创建器不得维护另一套不一致的调用协议。
- 默认 `start_expand.py` 只真实实现面板声明的 `refresh`，其他未知命令返回明确错误，不会伪造外部设备已经执行成功。
- `create` 传入自定义 `start_expand` 时，创建器会移除默认 action 容器，避免把 `refresh` 错配给不支持它的自定义入口；模块作者再按真实命令补回 action。
- 默认组件值和状态与 `input_data.md` 平行，不会自动进入 Prompt；模块代码必须显式、安全地消费配置。
