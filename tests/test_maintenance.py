...
class MaintenanceSchedulerTests(unittest.TestCase):
    def test_force_scan_promotes_expired_half_year_memory_to_permanent(self) -> None:
        ...
        self.assertEqual(result["alice"]["memory_lifecycle"]["upgraded"], [filename])
        self.assertTrue(store.fragment_path("permanent", filename).is_file())
        self.assertEqual(store.load_index("half_year"), {})
        self.assertEqual(
            result["alice"]["important_memory"]["reason"],
            "no_temporary_memory",
        )
