...
export const handlers = [
  ...
  http.get('/api/users/kesepain/overview', () => HttpResponse.json({
    ...
    counts: { sessions: 1, knowledge_documents: 2, enabled_tools: 2, enabled_agents: 1, active_tasks: 0 },
  ...
  http.get('/api/users/kesepain/knowledge', () => HttpResponse.json({ user: 'kesepain', enabled: true, retrieval: { max_items: 4, max_chars: 4000, minimum_score: 2, mode: 'file_index' }, summary: { documents: 2, user_documents: 1, global_documents: 1 }, documents: [{ scope: 'user', relative_path: 'notes.md', title: '个人笔记', size: 120, updated_at: 1 }], extensions: { kemo_graph: 'not_connected' } })),
  ...
  http.get('/api/users/kesepain/sense', () => HttpResponse.json({ user: 'kesepain', registry_available: false, injection_enabled: false, core_available: true, core_files: 1, summary: { registered: 0, enabled: 0, user: 0, shared: 0, project: 0 }, sources: [], decisions: [] })),
