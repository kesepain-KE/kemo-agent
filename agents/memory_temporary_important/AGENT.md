...
临时重要记忆子代理。它维护的是可重建的「热画像视图」，不是临时记忆到永久记忆之间的独立存储层。临时微记忆仍是权威来源，只能由用户对话历史整理继续加权，并正常到期和晋升；热画像只负责把近期高价值内容提前注入 Prompt，绝不反向影响源碎片权重。
...
- 热画像正文尽量控制在 `memory.important_memory_max_chars`（默认 20000 字符）以内；该值只是 Prompt 注入预算。`memory.important_memory_output_max_chars` 是语义独立的模型输出防失控硬上限，当前默认同为 20000 字符，超过时运行时会整体拒绝。不得收录没有对应临时源碎片的事实。
...
3. 优先精简表述并保持在 20000 字符以内；候选较多时信息完整性优先，但不得超过 20000 字符输出硬上限。
...
- 正文接近 20000 字符时仍应优先保留已确认的重要候选并精简重复表述；超过 20000 字符会被拒绝，下一次巡检再按正常完整数据重新整理。
