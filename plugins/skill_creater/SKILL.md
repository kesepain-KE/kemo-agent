...
技能用于可复用的任务方法，不是自动执行器。主智能体只注入发现摘要，正文按来源路径读取；配套脚本须通过已有获准工具执行，不注册 Provider 工具。创建前先查重并确认作用域和内容；编写规范见 `global_knowledge/prompt-authoring-standard.md`。结构化模式中的 `tool_schema` 仅生成参数文档。
...
用户主动要求**创建、修改或升级**技能时（`user_create` / `shared`，由主智能体发起），若该技能主题已有相关记忆，主智能体必须**先提醒用户**：

1. 用 `memory_manage action=search_many tier=all` 查询该技能主题相关的记忆（查询词用技能名、主题关键词 2～4 个）；
2. 命中后向用户说明：「发现 N 条与该技能相关的记忆：<文件名清单>。是否把这部分记忆融合进技能、或做相应整理？」
3. **默认保留记忆，不删除**。只有用户明确同意后才做后续处理；用户未表态时，技能照常创建/更新，记忆原样保留；
4. 用户拒绝融合时，不要再重复询问同一技能的同类记忆。

`self_improve` 在 180d→permanent 晋升时**后台自动创建**技能，不适用本节：不提醒、不询问，记忆与技能各自独立保留。
