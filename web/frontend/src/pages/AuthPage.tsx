...
import { ApiError, bootstrapAuth, loginAuth } from '../api/client'
  ...
  return (
    <main className="auth-screen">
      <section className="auth-panel" aria-label="Web 访问认证">
        ...
        {(error || bootstrapError) && <div className="auth-error" role="alert">{error || bootstrapError}</div>}
        {bootstrapping && <div className="auth-progress"><LoaderCircle size={17} className="spin" />正在验证链接中的访问令牌…</div>}

        <div className="auth-methods">
          {status.methods.token && (
            <form className="auth-form" onSubmit={submitToken}>
              <div className="auth-method-title"><KeyRound size={18} /><span><strong>访问令牌</strong><small>使用当前实例配置的 Token 建立会话</small></span></div>
              <label>
                <span>Token</span>
              ...
              <button className="auth-submit" type="submit" disabled={!token || Boolean(pending) || bootstrapping}>{pending === 'token' ? <><LoaderCircle size={16} className="spin" />正在验证</> : '使用令牌进入'}</button>
            </form>
          )}

          {status.methods.password && (
            <form className="auth-form" onSubmit={submitPassword}>
              <div className="auth-method-title"><UserRound size={18} /><span><strong>账号登录</strong><small>使用当前实例配置的用户名和密码</small></span></div>
              <label>
                <span>用户名</span>
              ...
              <button className="auth-submit" type="submit" disabled={!username || !password || Boolean(pending) || bootstrapping}>{pending === 'password' ? <><LoaderCircle size={16} className="spin" />正在登录</> : '登录工作区'}</button>
            </form>
          )}
        </div>
