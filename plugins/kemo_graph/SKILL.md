...
3. 工具参数只允许使用注册表中的 `library_id`。不得把对话中出现的绝对路径直接作为 `store_root` 或来源路径。
...
9. `upload` 只上传 Markdown 正文并标记为待整理；二进制文档应放入管理员注册的 `source_roots` 后执行 `sync`。
...
`operation_guide.operation` 支持：`activate/configuration_status/libraries/refresh/status/initialize/scan/sync/ingest/query/upload/documents/jobs/deactivate`。
...
```json
{
  ...
  "input_schema": {
    ...
    "properties": {
      ...
      "operation": {
        "type": "string",
        "enum": ["activate", "configuration_status", "libraries", "refresh", "status", "initialize", "scan", "sync", "ingest", "query", "upload", "documents", "jobs", "deactivate"]
      },
      "library_ids": {
        "type": "array",
        "items": {"type": "string"},
