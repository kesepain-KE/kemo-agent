...
- 主智能体和子代理的 Provider 输出解析错误进入最多 5 次的有界重试；取消、认证、输入校验和确定性协议错误不重试。`context_manage` 的专用 JSON 修复已耗尽时不再被外层重复放大。
- 上下文压缩前的记忆提取结果在同一次子代理重试中复用，避免重复写入记忆库；热画像和普通记忆仍按各自既有链路处理。
- Provider 诊断支持更多敏感字段别名、前缀 JSON 错误和循环引用保护，并限制递归深度、节点数、集合项数及消息扫描长度。
- 持久化 Provider 响应的 `id`、`request_id` 和 `model` 会先脱敏并限制长度，避免异常标识扩大历史记录。
...
- 子代理整体外部超时仍位于通用重试协调器之外；触发该超时时不会重新开始完整的 5 次重试预算，后续需单独设计总时长与单次尝试预算。
